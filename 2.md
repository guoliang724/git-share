...
<!-- detached from the master, at this point, do not commit a new commit -->
...
git reset --hard HEAD~1 or `hash` <!-- 1 -->

git revert HEAD <!-- 2  not yet specify the parent commit-->
git revert -m 1 HEAD
...
<!-- git branch --no-staged  view the unmerged branch -->
...
git reset --hard HEAD~6 <!--accidently reset-->
git reflog
git reset --hard `hashname`
...
1. done a commit
2. found some changes did not being included
3. doing the changes
4. git add .
5. git commit --amend -m `msg`
